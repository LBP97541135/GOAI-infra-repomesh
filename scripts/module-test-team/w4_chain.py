...
ISSUE_REQUIREMENT = (
    "pricing-fixture 的 calculate_total 需要按 tax_rate 计税"
    "（当前直接返回 subtotal，自带测试失败）。只改 pricing-fixture 仓。"
)
...
def read(issue):
    r = c.get(f"/issues/{issue}/discovery")
    r.raise_for_status()
    return r.json()
...
def wait_idle(issue, label, limit=600):
    ...
    while time.time() - t0 < limit:
        ...
        if d.get("step_state") != "running":
            ...
            print(f"  [{label}] step={d.get('step')} state={d.get('step_state')} ({elapsed:.0f}s)")
            return d
        time.sleep(3)
...
def trigger(issue, step, body):
    r = c.post(
        ...
        json={"created_by_agent_id": MANAGER, "idempotency_key": key(step), **body},
    )
...
if not issue_id:
    r = c.post(
        "/issues",
        json={
            "requirement_text": ISSUE_REQUIREMENT,
            "created_by_agent_id": MANAGER,
            "idempotency_key": key("issue"),
        },
    )
    print("issue", r.status_code, r.text[:200])
    r.raise_for_status()
...
if (d.get("approval") or {}).get("state") != "approved":
    ...
    r = c.post(
        ...
        json={
            ...
            "decision": "approved",
            "evidence_version": ev,
            "reason": "W4 验收：业务计划只覆盖 pricing-fixture",
            "adjustments": [],
        },
    )
    print("  approval ->", r.status_code, r.text[:200])
    r.raise_for_status()
    d = read(issue_id)
if not (d.get("plan") or {}).get("task_dag") and not d.get("plan_id"):
    d = trigger(issue_id, "plan", {})
plan = d.get("plan") or {}
repositories = [n.get("repository") for n in (plan.get("task_dag") or [])]
print("  plan repositories:", repositories, "error:", plan.get("error"))
if os.environ.get("W4_MATERIALIZE", "1") == "1":
    for attempt in range(1, 6):
        r = c.post(
            f"/issues/{issue_id}/discovery/materialize",
            json={"created_by_agent_id": MANAGER, "idempotency_key": f"w4-{RUN}-materialize"},
        )
        ...
        if r.status_code == 503 and "rooms" in r.text:
            ...
            continue
        break
result = {"issue_id": issue_id, "materialization": read(issue_id).get("materialization")}
