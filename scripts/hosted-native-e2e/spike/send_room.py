#!/usr/bin/env python3
"""Send one Matrix message as @admin into the team or leader room, @mentioning a member
(same shape as
src/repomesh/integrations/agentteams/matrix.py:178-187:
body = "<matrix id> <text>", m.mentions.user_ids).
...
def main() -> None:
    ...
    ap.add_argument(
        "--bare",
        action="store_true",
        help="body = text only; the mention travels in m.mentions (needed for /approve)",
    )
    args = ap.parse_args()
    ...
    url = (
        f"{MX}/rooms/{urllib.parse.quote(room, safe='')}"
