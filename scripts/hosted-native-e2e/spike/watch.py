#!/usr/bin/env python3
"""Wave-0 spike watcher: rooms (Matrix), shared drive (MinIO via the controller's mc),
worker container state.

Appends to output/hosted-native-e2e/2026-09-03/spike/watch.log and rooms.jsonl.
...
def main() -> None:
    ...
    while time.time() - started < args.minutes * 60 and not stop_file.exists():
        ...
        for label, room in (
            []
            if args.no_rooms
            else (("team", CONFIG["team_room_id"]), ("leader", CONFIG["leader_room_id"]))
        ):
            for ev in room_messages(room):
                ...
                if eid in seen:
                    continue
                seen.add(eid)
                ts = datetime.fromtimestamp(ev["origin_server_ts"] / 1000, tz=UTC).strftime(
                    "%H:%M:%S"
                )
        ...
        drive = "\n".join(
            sorted(
                line.split("STANDARD")[-1].strip() + " " + line.split()[3]
        ...
        if drive != last_drive:
            ...
            meta = sh(
                ["docker", "exec", CONTROLLER, "mc", "cat", f"{prefix}/{args.attempt}/meta.json"]
            )
            try:
                m = json.loads(meta)
                log(
                    f"[drive] meta.json status={m.get('status')} "
                    f"acknowledged_at={m.get('acknowledged_at')} "
                    f"submitted_at={m.get('submitted_at')} repomesh_block={'repomesh' in m}"
                )
            ...
            if "result.md" in drive:
                res = sh(
                    [
                        ...
                        "mc",
                        "cat",
                        f"{prefix}/{args.attempt}/result.md",
                    ]
                )
                log("[drive] result.md: " + res[:800].replace("\n", " / "))
        if args.review:
            ldrive = sh(
                ["docker", "exec", CONTROLLER, "mc", "ls", "-r", f"{prefix}/{args.review}/"]
            )
            ldrive = "\n".join(
                sorted(
                    line.split("STANDARD")[-1].strip() + " " + line.split()[3]
                    for line in ldrive.splitlines()
                    if "STANDARD" in line
                )
            )
                ...
                if "result.md" in ldrive:
                    res = sh(
                        [
                            ...
                            "mc",
                            "cat",
                            f"{prefix}/{args.review}/result.md",
                        ]
                    )
        ...
        wcmd = (
            ...
            f"[ -d /work/{args.attempt}/.git ] && "
            f"echo work_head=$(git -C /work/{args.attempt} rev-parse --short HEAD) "
            f"changed=$(git -C /work/{args.attempt} status --porcelain | wc -l | tr -d ' ') "
            f"|| echo no_workspace; "
            f"ls /root/.copaw-worker/agt-worker-dfb8a4cda6f7"
            f"/.copaw/workspaces/default/shared/tasks/ "
            f"2>/dev/null | tr '\\n' ' '; echo; "
            f"ls /root/.copaw-worker/agt-worker-dfb8a4cda6f7"
            f"/.copaw/workspaces/default/shared/tasks/{args.attempt}/ "
            f"2>/dev/null | tr '\\n' ' '"
        )
        wstate = "" if args.no_worker else sh(["docker", "exec", WORKER, "sh", "-c", wcmd]).strip()
        wstate = " | ".join(line.strip() for line in wstate.splitlines() if line.strip())
