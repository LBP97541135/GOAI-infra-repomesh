...
{ "requirement_text": "string",          // 必填，去空白后非空；即弹窗需求文本
  ...
  "idempotency_key": "string" }          // 必填：内容重放去重
```
...
- **请求体不收 `organization_id`**：工作区归属由 `created_by_agent_id` 所属组织唯一决定
  （即 §2 取值链第三级），收两份必然引入不一致窗口。
...
- `project_id` 由服务端从 `idempotency_key` 稳定派生（UUIDv5，命名空间常量 + key），
  故**相同 key 重放天然落在同一 (project_id, plan_version=1) 上**，撞
  `PlanSnapshotAlreadyExists` 唯一约束 → 返回既有 issue，**不重复创建**（响应 200，
  首次创建 201；两种响应体同形）。
- **幂等键生成责任在客户端**（主脑裁决补充）：每次**逻辑创建**生成一个新键（建议随机
  UUID），**重试沿用同键**。`project_id` 从 key 派生意味着键空间全局共享——客户端若用
  低熵键（如需求文本 hash、固定字符串）会造成**跨用户意外碰撞归并**（两个人的 issue
  变成同一个）。前端接线必须按此实现，服务端不做键熵校验（校验不了）。
