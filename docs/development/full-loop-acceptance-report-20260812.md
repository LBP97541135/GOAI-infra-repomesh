...
6. **界面绿 ≠ 链路通**：本轮已出四例——A-10（收据 `materialized` 而任务包没写、点名停在
   ...
   共同形状：**状态机说成功，物理世界没发生**。验收必须落到最外层的真实副作用——仓库
   分支、对象存储里的字节、容器日志里的动作——不能停在读模型。
