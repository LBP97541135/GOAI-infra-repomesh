...
- 文档状态：**终稿**。主链已于 2026-08-13 走通并回滚（详见 §7 终判）；全程与修复线交替
  推进，共十九次修复落地、九个「一次性事件死角」逐个填平
...
1. ~~B4 主链的执行段~~ **已全程达成**（issue `2ee58846`，见 §7.1 终判链条与留档）；
2. ~~B5 saga 真执行~~ **已达成**：saga 开出 revert PR #4，`merge_revert_pull_request` 被
   CI 门禁正确拦下，change_set 落 `manual_intervention`（详见 §7.1）；
3. **5c1b3567 / 35e66beb / 96896557 / 425efbbc 四个半执行标本的重放转正**——终局绿链改由
   全新 issue 走通，四标本**保全未清偿**，留作 A-8 收养语义与 A-10/A-13 重放的验证件；
3b. **多仓并发交付**——终局为控制变量收敛为单仓；「同一仓被两个 issue 同时改」的收养语义
   已由物化侧三次实证（㉑），但**未走到两仓同时进入交付段**；
...
| S34 | **A-18 二三面终极取证：证据面** | 「AGENT 自述验证状态：**已验证**」+ 测试记录 1 条 `exit 0` + **AGENT 回报原文全文**（含「**I have not verified that the tests pass**」——A-23 的主体归错同屏并列）+ 候选 HEAD/BASE + **PR 可点链接** + CI ✓ tests required success + 「merge gate：受阻 · head-bound governance decision is missing」 |
| S35 | **授权单（第一个不可逆动作前）** | 授权主体 / **绑定快照 `110405f1… · IMMUTABLE`** / 有效 30 分钟 / 写入范围「仅合并 PR #3」/ **HEAD-BOUND SHA `32c803fd2ddc`** / 「任一 SHA、契约或门禁结果变化都会使授权立即失效」 |
| S36 | **终点帧：全流程闭环达成** | issue 头部「Open · **1 仓已全部合并**」`delivered`；治理决策 READY + head 绑定 + 理由原文 |
| S37 | **B5-saga：merged 形态回滚** | 范围表「已 merge / **revert PR** / 逆序第 1 步 / **merge 27126b29f5e8**」与留档逐字对；琥珀条全文；三道闸齐 |
...
### 7.1 判据①：全流程 GUI 闭环——**达成**

**主链全程实证，无一步 curl 写入**（issue `2ee58846`，`repomesh-e2e-billing`）：

> GUI 建 issue → 需求分析（真 LLM，充分 0.90）→ 候选评分（5 候选）→ 三档分类 →
> **改档留痕**（四条，主体+时间戳+意见）→ 批准 → 生成计划 v1 → **物化并开工**（一按即过）
> → 建团 + teamRoom/leaderDM 双房间 → 派工点名入房 → **Runner 备工作区（真仓代码+冻结
> spec）** → **编码 agent 真写代码** → **`python scripts/run_tests.py` 真跑，exit 0** →
> **change_set 建立** → **候选分支推真仓** → **PR #3** → **CI `tests` 绿** → 决策夹亮起
> 「待治理放行」→ **证据面**（agent 回报原文 + CI + 候选 + 变更）→ **授权单**（绑定快照
> IMMUTABLE / 30 分钟 / 仅合并 PR #3 / HEAD-BOUND SHA）→ 批准 → **`delivery_auto` 自动
> merge** → **GUI 整 change set 回滚** → **saga 开出 revert PR #4**

**终局留档**（B5 范围表逐字对过）：

| 项 | 值 |
| --- | --- |
| change_set | `18dc502c-8012-4bf0-bbd1-6f0b567603c1` |
| 候选分支 | `repomesh/a762abba/9dfa78f2` |
| head / base | `32c803fd2ddc…` / `0a7683a9532c…` |
| PR | `catbobyman/repomesh-e2e-billing#3`（**MERGED** 2026-08-13T01:21:03Z） |
| **merge_sha** | **`27126b29f5e89b3ed4e81e5602cefb2abc7bf63f`** |
| revert PR | `#4` · `repomesh/revert/18dc502c/9dfa78f2-27126b29f5e8`（分支名逐字派生自 `{change_set8}/{repo8}-{merge_sha12}`，**D-2 的零存储幂等派生首次活体自证**） |

**B5-saga 定性：已完整执行且被正确拦截**（不是「未完成」）。saga 动作链状态如实——
`create_revert_pull_request: succeeded` → `merge_revert_pull_request: **failed**` →
`revalidate_changeset: pending`，change_set 落 `manual_intervention`。**revert 的 CI 红是
正确结果**：被 revert 的那次提交里同时修好了夹具仓的多币种基线红，撤销它，红就复活。
**系统没有因为「这是回滚、而且回滚已获批准」就免检**——回滚对话框琥珀条那句「revert 仍要
过自己的 CI 与门禁，不会因为回滚已获批准就免检」得到逐字兑现。§4.6 范围表两半就此验齐
（种子 B 的 `withhold` / 本轮的 `revert_pull_request`），「失败转人工」那半也一并实证。

### 7.2 判据②：一键打开——**过，但有一处空承诺**

compose console 冷路径首跑即通（三容器 healthy、自迁移、8180→200，`down -v` 作用域经
`--dry-run` 实证）。**但 `dev-up` 不含 Runner 阶段**（A-17）：执行面进程要按老配方手工拉，
且其路径前缀配方随部署形态漂移无人守护。**「一键打开」对执行段是空承诺**，未修。

### 7.3 这一天真正的发现

判据之外，本轮验收的价值集中在两处：

**（一）一次性事件死角——九例同族，架构级**。见 §7 第 9 条家族表。根因一句话：
**这套部署有眼睛没有手**——巡检（reconciler）读回 CI、写下状态，却从不据此推进；所有
「据此做决定」的动作（undraft、评门、合并）都挂在**本部署根本不存在的 webhook 通道**上。
于是它能准确地告诉你一切都就绪了，然后什么也不做。**本报告反复出现的「界面绿≠链路通」，
架构成因就在这里。**

**（二）产线的质量下限比连通性更值得记**。见 §7 与 B4 质量证据行：编码 agent 读懂并遵守
目标仓库 README 关于「假绿」的约定、拒绝写死税率；自识别跨仓舍入口径未决并随变更上报；
把 `tax_estimate` 标为 advisory 而让 billing 保持权威；发现 spec 自相矛盾时**做了并声明
「revert that file if the path restriction is the binding constraint」而非偷偷做或干脆
不做**。**最有力的一帧是验收方按「假绿」手法去查它有没有改断言取巧——查出来的是它把断言
由单币种加强为 EUR/JPY/GBP 三币种。**

### 7.4 缺陷总账

**已关闭**：A-1、A-3、A-5（改写）、A-9（翻案）、A-10、A-14、A-15、A-16、A-19、A-24、
B-11、B-12。
**待修（A 类）**：A-2（dev-up 收养库）、A-6（关闭候选）、A-8（㉑ 已合并，两标本收养重放
待验）、A-12（投影不收敛）、A-13 家族余项、**A-17（一键不含执行面）**、A-18（六面，㉖ 已
修其四）、**A-21（文档层未修：权限可重装、指令不能）**、A-22（已修待复验）、**A-23（证据
溯源，下批第一位）**、**A-25（等人却不告诉人：交付审批入口缺失，本轮以 `REQUIRED_APPROVALS=0`
配置绕过——配置绕过≠缺陷消失）**、A-9b。
**待修（B/C 类）**：B-13（封状态不补入口）、C-1、C-2、A-20（新端点默认不接异常翻译）、
gate 理由措辞（状态派生冒充原因）、基线红绑架增量交付、计划生成器编造不存在的模块名、
失败态无指路文案、`graph_edges` 恒空、种子与真仓对账。
**挂起观察哨**：A-11（org leader 上行 503）、c（worker 侧 mc 对 storage `Access Denied`，
威胁交付物上传）、d（leader 滞留点名不补投）、第九角（`MERGE_REQUESTED` 被 CI 重观测拉回
——本轮读模型自行收敛，未复现）。

### 7.5 环境干预清单（我们自己动过的每一处）

**这份终判的可信度，取决于把「链路自己走通的」和「我们扶着走通的」分清楚。** 下表列出
本轮为让链路继续而对环境做的全部人工干预。**凡标「绕过」者，其对应缺陷一律不因本轮走通
而关闭。**

| # | 干预 | 执行方 | 性质 | 对结论的影响 |
| --- | --- | --- | --- | --- |
| 1 | `REPOMESH_DELIVERY_REQUIRED_APPROVALS` 置 0，且因该值在建候选时已冻库，**另对候选行外科 UPDATE 置零** | 主脑 | **绕过** | **A-25 不关闭**：默认配置下「纯 GUI 走完交付审批」仍不可达 |
| 2 | 两标本收据回填 `prefix`/`plan_fingerprint`（35e66beb / 5c1b3567） | 主脑 | 一次性清偿 | 使重放可达；A-10 的修复另有正向实证 |
| 3 | 清除一条假 `accepted` 的 `scm_commands` 行（GitHub 200 但未生效） | 主脑 | 一次性清偿 | 解除幂等键对重试的阻塞；A-24/第七层照记 |
| 4 | 5 个存量 `openclaw` 资源经控制器 CLI 就地改 `copaw` | 主脑 | 环境修正 | runtime 配对坏死的存量清偿；代码侧修复另有实证（`f16925c`） |
| 5 | 僵尸清理：8 支死队墓碑 + 16 worker 删除 + 8 残留 leader 置 Stopped + 10 死容器移除 | 主脑（用户授权） | 环境清理 | 使巡检圈从约 16 分钟缩短；A-9 的翻案结论不依赖它 |
| 6 | MinIO 附加策略 `team-rm-console-shared`（`teams/*` 读写） | 主脑 | 环境修正 | A-16；注：该修复的实际意义后被缩小为「防 LLM 自救路径被拒」 |
| 7 | MinIO 凭据重注入（原值因命令替换转义错误带上了字面双引号） | 主脑 | 环境修正 | 属主脑自埋的环境错，非产品缺陷 |
| 8 | Runner 进程手工拉起 + 路径前缀映射改恒等 | 主脑 | **绕过** | **A-17 不关闭**：`dev-up` 仍不含执行面阶段 |
| 9 | 三夹具仓 `test_commands` / `test_paths` 回填（迁移 + SQL） | 主脑 | 配置补齐 | ㉗ 的数据侧前提；注入链路本身有独立实证 |
| 10 | catalog 中 checkout/billing 两行 URL 外科更新为真仓地址 | 主脑（验收前段） | 环境修正 | 使真仓交付成为可能；api/client 仍为占位符 |
| 11 | AgentTeams sidecar 转发器（8090/6167/9000） | 主脑（验收前段） | 环境修正 | 宿主→控制面三重墙的绕行；非产品路径 |

**验收方自身的写动作，同样列出**：①误建一个空文本 issue `d5f1149f`（`querySelector` 定位
失误，已入方法论）；②对 425efbbc 发起过**一次** curl 重放尝试（经主脑授权，结果 409、
**零副作用**，用于证明 API 侧同样不可达）。**除此之外，本轮全部交付动作均在 GUI 完成。**

### 7.6 推送建议

**主链已通，但不建议以当前状态推送**，理由两条：①A-25 使「纯 GUI 走完交付审批」在默认
配置（`REQUIRED_APPROVALS≥1`）下不可达，本轮是以改配置绕过的；②A-17 令「一键打开」对
执行段不成立。**这两条都不是主链缺陷，而是「能用」与「可交付」之间的距离。** 其余待修项
均已定位到代码级、有明确修法，不构成阻断。

**给下一轮的一句话**：这条链今天走通了，靠的是十九次修复、九个死角的逐个填平，以及
一次把隐含前提写成显式指令的需求文本。**它现在能跑，但它还不能自己跑给别人看。**

## 附录 A · 方法论沉淀（本轮）

> 十七条，按发现顺序编号。**验收自身的四次失误均原位留痕、不做事后美化**——
> 见第 7 条（旧渲染险些把已交付的修复记成缺失）、第 11 条（在被别人改过的工作区里取基线，
> 据此选错了整条终局路径）、第 17 条（读错字段层级，把「值为 null」当成「功能没跑」），
> 以及方法论之外的一次操作失误（`querySelector` 定位不当，误建了一个空文本 issue，
> 见 §7.5）。**保留它们正是这份报告可信度的来源**：一份从不出错的验收报告，通常只是
> 没有记录它的错误。
...
7. **验收自己也会看到旧渲染**（本轮救了我三次，建议置顶）：㉖ 落地后我第一次查 DAG 节点，未验证
   ...
   三次都是「先刷新再判断」把误判挡住的。
8. **好文案是把判断权还给人**（S26 重新派工弹窗，本轮质量标杆）：该弹窗把「会不会写」
...
9. **200 不等于做了：写动作必须后验证**（第七层，本轮「证据可信度」主线的收束）：`undraft`
   的调用方修好之后，命令入队、派发、**GitHub 返回 200、命令行落账 `status=accepted`**——
   而 PR 仍是 draft。原因是 **GitHub REST 的 PATCH 不支持翻 draft 标志，它静默吞掉该字段
   并照样返回 200**。于是产线拿到一个「成功」的回执，记了下来，还用它的幂等键挡住了重试。
   **这一层的假成功不来自我们的代码，来自外部 API 本身**——但后果一样：状态机说成功，
   物理世界没发生。**规律**：**平台对外部系统的每个写动作，与 agent 对平台的每个报告，
   同样都需要后验证**（本条与 A-23「谁跑的测试」并列——一个是别人报给我们的成功，一个是
   我们报给自己的成功，都得回头看一眼真的发生了没有）。这也是这条战线一整天的同一条
   军规：**绝不报告未经验证的成功。**
10. **同一件事，靠自觉会失败，靠指令会成功**（本轮最有产品价值的对照，绿链的钥匙）：两个
...
11. **测基线要在没人动过的地方测**（验收自身的第三次失误）：我曾据一次实跑断言「billing
...
13. **一次性事件死角：本轮最有分量的架构级发现（五例同族）**。这条链上每一次「跨系统副
   ...
   | 6 | **undraft（调用方）** | **CI 已绿而 PR 永远停在 draft**：`undraft_when_allowed` 的唯一调用方是 `observation_processor:136`——**webhook 事件路径**；而本环境无 webhook，CI 状态是 reconciler 轮询自己读回并直接落账的（绕过 processor），**于是 undraft 永远没人调**（8100 日志零 `undraft`/`ready_for_review` 记录佐证）。活体标本 S33 | ㉚ 续修（同一接缝加 undraft 步，幂等） |
   | 8 | **算门禁并合并** | PR 已非 draft、CI 已绿、前置全齐，而 `merge_gate` 恒 `null`、candidate 恒 `review_pending`。根因与第 6 例**完全同构**：`merge_ready_repositories`（前向决定：逐候选评门→允许即 merge）的唯一调用方同样是 `observation_processor:137` 的 **webhook 路径**；**巡检至今只观察不决定**——它的 `evaluate_merge_gate` 只在「远端已 merge 的安全校验」分支里跑 | 修复续派（sweep 尾部补 decide 步） |
   | 7 | **undraft（外部 API）** | 修好调用方之后仍是 draft：sweep 触发、命令入队、dispatcher 派发、**GitHub 返 200**（`delivery.scm_commands` 里 `status=accepted` 铁证）——但 **GitHub REST 的 PATCH 根本不支持翻 draft 标志，它静默吞掉该字段并返回 200**；转正只有 GraphQL mutation 一条路。**即 undraft 在本产品的任何路径上（含 webhook 时代）从未真正工作过。** 连带：这条假 `accepted` 的命令行会以同幂等键挡住重试 | 修复续派（改 GraphQL mutation + **后验证 `isDraft`，拒绝静默无效**）；假 accepted 行由主脑一次性外科清除让 sweep 重新入队 |

   **共同界面面孔**：一个不说明在等什么的 `waiting` / 「门禁运行中」（A-18 第六面）。
   **第 6 与第 8 例多出一层**：它们挂的不只是「一次性触发」，而是**一条这套部署里根本不
   存在的触发通道**（webhook）——同一份 CI 状态，轮询路径读得到、事件路径永不到达，
   **功能因此悬在一条从未接通的线上**。判别法：问「这个动作的调用方是谁」，如果答案是
   某个事件处理器，再问一句「这个事件在本部署里真的会来吗」。
   **第 8 例还揭出一条更本质的分工失衡**：巡检（reconciler）**只观察不决定**——它读回
   CI、写下状态，却从不据此推进；所有「据此做决定」的动作（undraft、评门、合并）都挂在
   事件路径上。**于是这套部署有眼睛没有手**：它能准确地告诉你一切都就绪了，然后什么也
   不做。这正是「界面绿≠链路通」在架构层的成因。
...
14. **配置随部署形态漂移，而没有任何一层守护**（A-17 后半）：Runner 的路径前缀映射是
   「API 在容器里」年代的配方，部署形态改成「API 在宿主」后它每单都判不可解析、静静
   放掉，只续租不开工。**症状是「安静地续租」而不是报错**——这类漂移不会自己喊疼，
   只能靠对账（派单在租、事件流水为空 = 有人租了活却没干）发现。
15. **子串匹配的安全规则会咬自己的命名规范**（A-15）：`TOOL_CMD_DANGEROUS_RM` 以子串匹配
   `rm`，而本系统所有资源名都是 `rm-` 前缀，于是每一次 `cat …/rm-worker-…/…` 都被判成
   危险删除。**命名空间与安全词表要一起设计**；规则用词边界，不用子串。
16. **两端各说各话的配置**（A-14 系统性观察）：能力挂载是条件性的（url 空则静默跳过），
   而指令文案是无条件的。任何「A 侧可选、B 侧必需」的配对都需要一道一致性校验，否则
   缺失只会在最远端以「什么都没发生」的形式出现。
17. **读值之前先确认这个字段住在哪一层**（验收自身的第四次失误）：我连续两轮报告
   「`merge_gate` 从未被计算」，据此推断出一个并不存在的死角——实际上判定一直在
   **`change_set.repositories[].merge_gate`** 里算得好好的，而我读的是交付聚合的**顶层
   同名字段**（该字段恒 `null`）。**同名不同层，比字段缺失更难发现**：它给你一个语法正确、
   语义完全错误的答案。与第 7 条（先确认这版数据什么时候取的）互为表里——**一个管时间，
   一个管位置，取值前两样都要确认。**
