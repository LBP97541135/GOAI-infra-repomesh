...
> **⚠ 本节的落点设想已被细化，动工前先读
> [`supervision-policy-window-20260814.md`](supervision-policy-window-20260814.md)。**
> 5-1a（只读）已完成并合入。5-1b（可设）摸下来发现一个时序死结：设策略要求仓库**已有**
> AI 成员，而 AI 成员是**物化时才创建**的，物化又同时把策略钉成 `auto` + 零卡点且无更新
> 端点——可设窗口只有「仓库已建团 ∧ 尚未物化」这一段。三个方案的比较、被否决的那个
> （让策略搭物化的车，会绕过 `is_admin`）、以及界面形状被域不变量决定的部分，全在那份
> 文档里。
