...
Usage:
  build_package.py construction --attempt-id <uuid> --generation N --out <root> [--previous-note TEXT]
  build_package.py review --attempt-id <uuid> --review-of <construction attempt uuid> --candidate-dir <dir> --out <root>
...
def build_construction(args) -> Path:
    ...
    meta = {
        ...
        "task_title": f"Implement multi-currency quote() for {CONFIG['repository']} (attempt {args.generation})",
...
def build_review(args) -> Path:
    ...
    values = {
        ...
        "changed_files": "\n".join(f"- `{c['status']}` `{c['path']}`" for c in changes["changed_files"]),
    ...
    meta = {
        "task_id": args.attempt_id,
        "project_id": CONFIG["project_id"],
        "task_title": f"Review candidate {changes['head_sha'][:8]} for {CONFIG['repository']} (attempt {args.review_of[:8]})",
