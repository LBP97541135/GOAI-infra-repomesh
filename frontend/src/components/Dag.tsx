 ...
 *  room = 暗色房间流，paper = 计划纸面。状态由 display_status 透传，仅渲染。 */
...
function statusLabel(t: DeliveryTask): string {
  switch (t.status) {
    case "succeeded":
      return "已完成";
    case "running":
...
export function Dag({
  ...
  const height = Math.max(322, 52 + lanes.length * 72 - 20);

  return (
    <svg className="block h-auto w-full" viewBox={`0 0 980 ${height}`} role="img" aria-label="跨仓任务 DAG">
      {lanes.map((name, i) => (
        <g key={name}>
          <line
            x1={18}
