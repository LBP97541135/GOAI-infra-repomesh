      ...
      <div className="mx-auto max-w-[1040px]">
        ...
        <div className="mt-5 grid gap-6 lg:grid-cols-[220px_minmax(0,1fr)]">
          ...
          <main className="min-w-0">
            ...
            {step === 2 ? <section><h2 className="text-[15px] font-semibold text-cream">GitHub App 配置</h2><p className="mt-1 mb-3 text-[11.5px] text-tx2">可跳过；需要 GitHub 自动交付或 webhook 时再配置。推荐用一键创建：登录 GitHub 确认后凭证自动回填。</p>{credentials ? <GitHubAppCredentialForm status={credentials.github_app} registration={credentials.github_app_registration} onSaved={saved} /> : <p className="text-tx3">读取中…</p>}</section> : null}
