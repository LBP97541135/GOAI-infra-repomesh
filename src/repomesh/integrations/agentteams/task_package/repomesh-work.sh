#!/usr/bin/env bash
# repomesh-work.sh -- RepoMesh hosted-native construction helper (task package v2).
#
#   bash base/tools/repomesh-work.sh init     create the local workspace from base/base.bundle at the base commit
#   bash base/tools/repomesh-work.sh test     run the frozen test commands inside the workspace and record the results
#   bash base/tools/repomesh-work.sh bundle   write candidate/{candidate.bundle,candidate.diff,changes.json,evidence.json}
#   bash base/tools/repomesh-work.sh clean    delete the local workspace (platform / operator use)
#
# The workspace lives OUTSIDE the task directory (default <workspace_root>/<attempt_id>, workspace_root from
# base/package.json, normally /work) so that the task directory never contains a repository checkout, .git,
# dependencies or build caches. REPOMESH_WORK_ROOT overrides the root.
# This script carries no credentials and never talks to the network.
# Contract: contracts/agentteams-task/v2/helper-cli.md
set -euo pipefail
...
die() { echo "repomesh-work: error: $*" >&2; exit 2; }
note() { echo "repomesh-work: $*"; }

[ -f "$PACKAGE" ] || die "base/package.json not found (expected at $PACKAGE)"
command -v git >/dev/null 2>&1 || die "git is required"
command -v python3 >/dev/null 2>&1 || die "python3 is required"

# pkg <dotted.key> [default]: read one value from base/package.json (lists print one item per line).
pkg() {
  python3 - "$PACKAGE" "$1" "${2-}" <<'PY'
import json, sys
...
for part in sys.argv[2].split("."):
    if not isinstance(node, dict) or part not in node:
        if len(sys.argv) > 3 and sys.argv[3] != "":
            print(sys.argv[3])
            sys.exit(0)
        sys.exit(f"repomesh-work: error: base/package.json has no key {sys.argv[2]}")
    node = node[part]
...
WORK_ROOT="${REPOMESH_WORK_ROOT:-$(pkg workspace_root /work)}"
...
STATE_DIR="$WORK_ROOT/.repomesh-work-state/$ATTEMPT_ID"
...
if failed:
    print(f"repomesh-work: {len(failed)} of {len(data['results'])} test command(s) FAILED; fix the code and run test again")
    sys.exit(1)
print(f"repomesh-work: all tests passed ({len(data['results'])} command(s)); next: bash base/tools/repomesh-work.sh bundle")
PY
}
