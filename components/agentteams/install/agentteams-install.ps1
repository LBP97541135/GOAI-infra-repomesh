...
# Matrix AppService (controller <-> Matrix bridge; tokens auto-generated by installer)
...
function Install-Manager {
    ...
    # Matrix AppService (controller <-> Matrix bridge): enabled by default with
    # auto-generated tokens (mirrors agentteams-install.sh). Persisted to the env
    # file so tokens don't drift across upgrades. The controller panics when
    # AppService mode is on but AS/HS tokens are empty, so these must always be set.
    $env:AGENTTEAMS_MATRIX_APPSERVICE_ENABLED = if ($env:AGENTTEAMS_MATRIX_APPSERVICE_ENABLED) { $env:AGENTTEAMS_MATRIX_APPSERVICE_ENABLED } else { "true" }
    $env:AGENTTEAMS_MATRIX_APPSERVICE_AS_TOKEN = if ($env:AGENTTEAMS_MATRIX_APPSERVICE_AS_TOKEN) { $env:AGENTTEAMS_MATRIX_APPSERVICE_AS_TOKEN } else { New-RandomKey }
    $env:AGENTTEAMS_MATRIX_APPSERVICE_HS_TOKEN = if ($env:AGENTTEAMS_MATRIX_APPSERVICE_HS_TOKEN) { $env:AGENTTEAMS_MATRIX_APPSERVICE_HS_TOKEN } else { New-RandomKey }
